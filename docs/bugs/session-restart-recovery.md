...
已实现安全重绑：服务重启、TTL 或容量淘汰后，旧页面下一次状态型操作会按稳定的
浏览器会话身份创建新服务端会话；旧 owner token 和旧业务字典不会被直接接受，
因此不会再重复抛出 no server session id。周期拼接草稿以 24 小时保留期持久化，
恢复时换发新服务端身份并重建派生预览。

当前实现采用“下一次用户操作触发恢复”，没有增加全局心跳轮询；智能分割、PVS、
版图等未持久化工作流在重绑后安全初始化为空状态，需要重新提交输入。
