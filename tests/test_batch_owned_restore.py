...
class BatchOwnedRestoreTests(unittest.TestCase):
    def test_four_image_restore_keeps_guarded_mode_callback_usable(self):
        session = app._SESSION_REGISTRY.bind("batch-owned-test", "10.0.0.1")
        request = gr.Request(session_hash="batch-owned-test", client=SimpleNamespace(host="10.0.0.1"), headers={})
